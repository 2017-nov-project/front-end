...
class App extends Component {
  ...
  handleChange = async (userInput) => {
    ...
    if (/\d/.test(userInput)) {
      ...
      const types = await Promise.all(['town', 'locality'].map(async (searchType) => {
        const results = await fetch(`https://peaceful-waters-20110.herokuapp.com/api/${searchType}/${userInput}`)
    ...
    getAveragePriceByInput(userInput, searchType)
      .then(({ average }) => this.setState({ userInput, searchType, averagePrice: average }))
  }
  ...
  render() {
    ...
    console.log(`render called with`, { userInput, searchType });
    ...
    return (
      <div className="App">
        ...
        <ChartType userInput={userInput} searchType={searchType} />
        <div className='wrapper'>
          <div className='inputrow'>
            <form><input required title="3 characters minimum" onBlur={(e) => this.handleChange(e.target.value)} className='postcodeInput' placeholder='enter postcode or town' type="text"></input>
